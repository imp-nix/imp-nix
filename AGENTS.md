...
- `<ID>` values:
  ...
  - `FS_MODEL` (shared filesystem semantics and entry classification)
  ...
  - `COLLECT_KEYED` (shared dotted-key declaration flattening)
  - `COLLECT_HOSTS` (host declaration collection)
  - `BUILD_OUTPUTS` (output materialization)
  - `RECORD_STRATEGIES` (shared strategy analysis for record merging)
